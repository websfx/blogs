...
解决这个问题方法有很多种，具体可以参考 [PHP并发IO编程之路](http://rango.swoole.com/archives/508), 看看前半段就行了，后半段是广告！该文章总结了3种方法：最早是采用多进程多线程方式，由于进程线程开销大，这种方式效率最低。后来演进出master-worker模型，也就是类似现在fpm采用的方式。目前最先进的方式就是异步io多路复用，基于epoll实现的。

理论上讲C能实现的，PHP都能通过扩展去实现，其思想都差不多，然而今天咱不是说高并发编程的，还是接着说Socket吧！
...
socket_close($sock);
```
这些代码和上面的区别就是我按照自己的需求定义了一个“协议”，在这个例子里面，我把需要运算的数和方式通过一个json数组传输，因为这个协议只有我自己清楚，所以只有我们才知道怎么调用。

这个例子还有问题，客户端依然是一次性的，参数都被硬编码在代码里面，不够灵活，最关键是没有用到TCP长链接的特性，我们需要的是一次链接，多次对话！

>不知道大家看到现在有没有发现一个问题，那就是目前为止，这些代码都没有复用链接，因为在服务端最后我close了这个链接，这意味着每次都是一个新的请求，如果是http服务的话尚且可以用一下，但是这并不是我想要实现的效果！

### 4.IO多路复用之select
select系统调用的目的是：在一段指定时间内，监听用户感兴趣的文件描述符上的可读、可写和异常事件，虽然这个方式也比较低效，但是不妨了解一下，通过这种方式我们可以复用链接，完整的代码如下：
```php
<?php
set_time_limit(0);
...
![](http://ww1.sinaimg.cn/large/5f6e3e27ly1g37t6zlv03j20cq07iq38.jpg)

在这个例子，传参的“协议”稍微有点变化，但是很容易理解 
